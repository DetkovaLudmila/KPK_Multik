// {----------------------------------------------- ------------------------------
// ! @brief Function Library на C ++ для разработки анимации
// !
// ! @ автор Абрамова Ирина,
// ! Краснодар, гимназия 36
// !
// !
// ! @date май 2021 г.
// !
...
// ! компонент RGB.
// !
// ! \ image html Entry1.png Ширина "SpringFon" = 250 пикселей
// !
// ! @see SpringFonBegining (); SpringFon ();
// !
// ! @note Неплохо дополняет данные функции такими объектами, как: Cloud_draw (), Tree_draw (),
...
// {----------------------------------------------- -----------------------------------------------
// ! Эта функция также рисует ясное градиентное небо и зеленый холм.
// !
...
// !
// ! \ image html Mountain.png "Розовые горы"
// !
// ! @see Reed_draw (); Water_lily_draw ()
// !
// ! @note Неплохо дополнить функцию функции объекта: Water_lily_draw (), Reed_draw ()
...
// ! Параметры отсутствуют.
// !
// ! \ image html Savanah.png "Земли Саваны"
// !
// ! @see Cactus_draw (); BigDipper_draw ();
// !
// ! @note Неплохо дополнить функцию функции объекта: Cactus_draw (), BigDipper_draw ()
...
// !
// ! @param xx - координата первого эллипса
// ! @param yy - координата первого эллипса
...
// !
// ! @param xx - координаты верхушки ствола
// ! @param yy - координаты верхушки ствола
...
// !
// ! @param xx - координата центра лепестка
// ! @param yy - координата центра лепестка
...
// {----------------------------------------------- -----------------------------------------------
// ! Эта функция рисует кактус. Заметное дополнение к ночной пустыне!
// !
// ! \ image html Cactus.png "Кактус обыкновенный" width = 280px
// !
// ! @param xx - координата центра ствола
// ! @param yy - координата центра ствола
// ! @param sizeX Размер кактуса (доп. толщина ствола)
// ! @param sizeY Размер кактуса
// ! @param COLORREF cactusColor Цвет кактуса

// ! @note Замечательно сочетается с цветочками! (Цветочный_ рисунок ())
// !
// !
// ! @ см. EMO_Cactus_draw ()
// !
...
// !
// ! @param xx - координата центра цветка
// ! @param yy - координата центра цветка
...
// !
// ! @код
// ! Цветочный рисунок (850, 565, 0,5, 0,5, RGB (255, 203, 255));
// ! @endcode
// } ----------------------------------------------- -----------------------------------------------
...
// !
// ! @param xx - координата центра первой окружности
// ! @param yy - координата центра первой окружности
// ! @param sizeX Размер камыша
// ! @param sizeY Размер камыша
// ! @param покачивается Наклон камыша
// !
// !
// !
// ! @see Water_lily_draw ()
...
// !
// ! @param xx - координата первой точки полигона куста
// ! @param yy - координата первой точки полигона куста
...
// !
// ! @param xx - координата цетра головы бабочки
// ! @param yy - координата цетра головы бабочки
// ! Размер бабочки (радиус окружностей, составных частей бабочки)
...
// !
// ! @param xx - координата верхней точки головы
// ! @param yy - координата верхней точки головы
// ! @param sizeX Размер птички
// ! @param sizeY Размер птички
// ! @param брови Подьем бровей
...
// !
// ! @param xx - координата первой "мышки"
// ! @param yy - координата первой "мышки"
...
// !
// ! @param xx - координата "шеи" мышки
// ! @param yy - координата "шеи" мышки
// ! @param width Размер мышки (по x)
// ! @param height Размер мышки (по y)
// ! @param COLORREF bodyColor Цвет мышки
// ! @param COLORREF ушекColor Цвет ушек мышки
// !
// ! @ см. Mouse_twerk ()
// !
// ! @par Пример использования:
...
// !
// ! @param xx - координата центра головы
// ! @param yy - координата центра головы
// ! @param width Размер котика
// ! @param height Размер котика
// ! @param брови Подьем бровей котика
...
// !
// ! @param xx - координата верхней точки мельницы
// ! @param yy - координата верхней точки мельницы
...
// ! \ image html Mill.png "Мельница"
// !
// ! @param x0 x0 - верхняя точка крыла
// ! @param y0 y0 - верхняя точка крыла
// ! @param xx - нижняя точка крыла
// ! @param yy - нижняя точка крыла
// !
// ! @see WindMill_draw ()
// !
// ! @par Пример использования:
// !
// ! @код
// ! Millblades_draw (675, 180, КРУГЛЫЙ (100 * cos (t * 0,1) + 675), КРУГЛЫЙ (100 * sin (t * 0,1) + 180));
// ! @endcode
// } ----------------------------------------------- -----------------------------------------------
...
//{----------------------------------------------------------------------------------------------
//!         Р­С‚Р° С„СѓРЅРєС†РёСЏ СЂРёСЃСѓРµС‚ СЃРѕР·РІРµР·РґРёРµ " Р‘РѕР»СЊС€Р°СЏ РјРµРґРІРµРґРёС†Р°"
//!
//! \image html BigDipper.png "Р‘РѕР»СЊС€Р°СЏ РјРµРґРІРµРґРёС†Р°"
//!
//! @param  x           x - С†РµРЅС‚СЂ РїРµСЂРІРѕР№ Р·РІРµР·РґС‹
//! @param  y           y - С†РµРЅС‚СЂ РїРµСЂРІРѕР№ Р·РІРµР·РґС‹
//! @param  r           СЂР°РґРёСѓСЃ Р·РІРµР·РґС‹
//!
...
//{----------------------------------------------------------------------------------------------
//!         Р­С‚Р° С„СѓРЅРєС†РёСЏ СЂРёСЃСѓРµС‚ СЃРєРµР№С‚Р±РѕР°СЂРґ (Р±РµР· РєРѕР»РµСЃ)
//!
//! \image html Skate.png "РЎРєРµР№С‚Р±РѕР°СЂРґ"
//!
//! @param   x          x - РєРѕРѕСЂРґРёРЅР°С‚Р° Р»РµРІРѕРіРѕ СѓРіР»Р° СЌР»Р»РёРїСЃР°
//! @param   y          y - РєРѕРѕСЂРґРёРЅР°С‚Р° Р»РµРІРѕРіРѕ СѓРіР»Р° СЌР»Р»РёРїСЃР°
//! @param   sizeX      Р Р°Р·РјРµСЂ РґРѕСЃРєРё
//! @param   sizeY      Р Р°Р·РјРµСЂ РґРѕСЃРєРё
//!
...
//{----------------------------------------------------------------------------------------------
//!         Р­С‚Р° С„СѓРЅРєС†РёСЏ СЂРёСЃСѓРµС‚ РєРѕР»РµСЃР° РґР»СЏ СЃРєРµР№С‚Р±РѕР°СЂРґР°
//!
//! \image html Skate.png "РЎРєРµР№С‚Р±РѕР°СЂРґ"
//!
//! @param   x0          x - РєРѕРѕСЂРґРёРЅР°С‚Р° С†РµРЅС‚СЂР° РїРµСЂРІРѕРіРѕ РєРѕР»РµСЃР°
//! @param   y0          y - РєРѕРѕСЂРґРёРЅР°С‚Р° С†РµРЅС‚СЂР° РїРµСЂРІРѕРіРѕ РєРѕР»РµСЃР°
//! @param   r           Р Р°РґРёСѓСЃ РєРѕР»РµСЃ
//!
//!
//! @see     Skateboard_draw()
//!
...
//{----------------------------------------------------------------------------------------------
//!         Р­С‚Р° С„СѓРЅРєС†РёСЏ СЂРёСЃСѓРµС‚ РїСЂРѕСЃС‚Рѕ РїР°Р»РєСѓ.
//!
//! \image html Stick.png "РњРЅРѕРіРѕС„СѓРЅРєС†РёРѕРЅР°Р»СЊРЅР°СЏ РїР°Р»РєР°"
//!
//! @param   x               x - РєРѕРѕСЂРґРёРЅР°С‚Р° РІРµСЂС…РЅРµР№ С‚РѕС‡РєРё РїР°Р»РєРё
//! @param   y               y - РєРѕРѕСЂРґРёРЅР°С‚Р° РІРµСЂС…РЅРµР№ С‚РѕС‡РєРё РїР°Р»РєРё
//! @param   sizeX           Р Р°Р·РјРµСЂ РїР°Р»РєРё
//! @param   sizeY           Р Р°Р·РјРµСЂ РїР°Р»РєРё
//! @param   woodenarms      РџРѕР»РѕР¶РµРЅРёРµ РїР°Р»РєРё
//!
//! @par     РџСЂРёРјРµСЂ РёСЃРїРѕР»СЊР·РѕРІР°РЅРёСЏ:
//!
//! @code
...
//{----------------------------------------------------------------------------------------------
//!         Р­С‚Р° С„СѓРЅРєС†РёСЏ СЂРёСЃСѓРµС‚ РєСѓСЃРѕРє СЃС‹СЂР°.
//!
//! \image html Cheese.png "РљСѓСЃРѕРє СЃС‹СЂР°"
//!
//! @param   x               x - РєРѕРѕСЂРґРёРЅР°С‚Р° РєСЂР°Р№РЅРµР№ Р»РµРІРѕР№ С‚РѕС‡РєРё РєСѓСЃРєР°
//! @param   y               y - РєРѕРѕСЂРґРёРЅР°С‚Р° РєСЂР°Р№РЅРµР№ Р»РµРІРѕР№ С‚РѕС‡РєРё РєСѓСЃРєР°
//! @param   bigness         Р Р°Р·РјРµСЂ РєСѓСЃРєР° СЃС‹СЂР°
//!
//! @par     РџСЂРёРјРµСЂ РёСЃРїРѕР»СЊР·РѕРІР°РЅРёСЏ:
//!
//! @code
...
//{----------------------------------------------------------------------------------------------
//!         Р­С‚Р° С„СѓРЅРєС†РёСЏ СЂРёСЃСѓРµС‚ РґРµРІРѕС‡РєСѓ.
//!
//! \image html Girl.png "Р”РµРІРѕС‡РєР°"
//!
//! @param   x                  x - РєРѕРѕСЂРґРёРЅР°С‚Р° С€РµРё РґРµРІРѕС‡РєРё
//! @param   y                  y - РєРѕРѕСЂРґРёРЅР°С‚Р° С€РµРё РґРµРІРѕС‡РєРё
//! @param   sizeX              Р Р°Р·РјРµСЂ РґРµРІРѕС‡РєРё
//! @param   sizeY              Р Р°Р·РјРµСЂ РґРµРІРѕС‡РєРё
//! @param   hands_UP           РР·РјРµРЅРµРЅРёРµ РїРѕР»РѕР¶РµРЅРёСЏ  СЂСѓРє
//! @param   legs_DISTANCE      РР·РјРµРЅРµРЅРёРµ СЂР°СЃС‚РѕСЏРЅРёСЏ РјРµР¶РґСѓ РЅРѕРіР°РјРё
//! @param   eyebrows_UP        РџРѕР»РѕР¶РµРЅРёРµ Р±СЂРѕРІРµР№
//! @param   eyesCRAZYleft      Р Р°Р·РјРµСЂ Р»РµРІРѕРіРѕ РіР»Р°Р·Р°
//! @param   eyesPUPILleft      Р Р°Р·РјРµСЂ Р»РµРІРѕРіРѕ Р·СЂР°С‡РєР°
//! @param   eyesCRAZYright     Р Р°Р·РјРµСЂ РїСЂР°РІРѕРіРѕ РіР»Р°Р·Р°
//! @param   eyesPUPILright     Р Р°Р·РјРµСЂ РїСЂР°РІРѕРіРѕ Р·СЂР°С‡РєР°
//! @param   mouthTOPpoint      РџРѕР»РѕР¶РµРЅРёРµ РІРµСЂС…РЅРµР№ С‚РѕС‡РєРё СЂС‚Р°
//! @param   mouthLOWERpoint    РџРѕР»РѕР¶РµРЅРёРµ РЅРёР¶РЅРµР№ С‚РѕС‡РєРё СЂС‚Р°
//! @param   COLORREF HairColor Р¦РІРµС‚ РІРѕР»РѕСЃ
//! @param   COLORREF BodyColor Р¦РІРµС‚ С‚РµР»Р°
//!
//!
//! @par      РџСЂРёРјРµСЂ РёСЃРїРѕР»СЊР·РѕРІР°РЅРёСЏ:
//!
...
//{----------------------------------------------------------------------------------------------
//!         Р­С‚Р° С„СѓРЅРєС†РёСЏ СЂРёСЃСѓРµС‚ РєР°РєС‚СѓСЃ СЃ СЂРѕР¶РёС†РµР№ РІ С†РІРµС‚РѕС‡РЅРѕРј РіРѕСЂС€РєРµ.
//!
//! \image html Emo.png "Р­РјРѕ - РєР°РєС‚СѓСЃ"
//!
//! @param   x                  x - РєРѕРѕСЂРґРёРЅР°С‚Р° Р»РµРІРѕР№ РєСЂР°Р№РЅРµР№ С‚РѕС‡РєРё РіРѕСЂС€РєР°
//! @param   y                  y - РєРѕРѕСЂРґРёРЅР°С‚Р° Р»РµРІРѕР№ РєСЂР°Р№РЅРµР№ С‚РѕС‡РєРё РіРѕСЂС€РєР°
//! @param   sizeX              Р Р°Р·РјРµСЂ РєРѕРјРїРѕР·РёС†РёРё
//! @param   sizeY              Р Р°Р·РјРµСЂ РєРѕРјРїРѕР·РёС†РёРё
//! @param   eyebrows_UP        РџРѕР»РѕР¶РµРЅРёРµ Р±СЂРѕРІРµР№
//! @param   eyesCRAZYleft      Р Р°Р·РјРµСЂ Р»РµРІРѕРіРѕ РіР»Р°Р·Р°
//! @param   eyesPUPILleft      Р Р°Р·РјРµСЂ Р»РµРІРѕРіРѕ Р·СЂР°С‡РєР°
//! @param   eyesCRAZYright     Р Р°Р·РјРµСЂ РїСЂР°РІРѕРіРѕ РіР»Р°Р·Р°
